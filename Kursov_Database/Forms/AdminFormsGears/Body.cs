...
namespace Kursov_Database.Forms.AdminFormsGears
{
    public partial class Body : Form
    {
        ...
        void ShowAllPerks()
        {
            ...
            Table = new DataTable();
            Adapter = new MySqlDataAdapter("select `Name` from `kursach_database`.body;", Connection);
            Adapter.Fill(Table);
        ...
        void RefreshGearTable()
        {
            ...
            ConnectToDatabase("root", "root");
            Table = new DataTable();
            Adapter = new MySqlDataAdapter("select name from body;", Connection);
            Adapter.Fill(Table);
            AllGearsTable.DataSource = Table;
            DisconnectFromDatabase();
        }
        ...
        private void AddGearButton_Click_1(object sender, EventArgs e)
        {
            ...
            if (!(NameTextbox.Text == ""))
            {
                try
                {
                    ...
                    ConnectToDatabase("root", "root");
                    string commandText = String.Format("insert into Body (Name, Type, AllowedClass, Gearscore) values ('{0}','{1}','{2}',{3});", NameTextbox.Text, checkedListBox1.CheckedItems[0], AllClasses, Convert.ToInt32(NumberTextbox.Text));
        ...
        private void ChangeButton_Click(object sender, EventArgs e)
        {
            if (!(NameTextbox.Text == ""))
            {
                try
                {
                    ...
                    string commandText = String.Format("UPDATE Body SET Name= '{0}', Type = '{1}', AllowedClass = '{2}', Gearscore = {3} where Name = '{4}';", NameTextbox.Text, checkedListBox1.CheckedItems[0], AllClasses, Convert.ToInt32(NumberTextbox.Text), value.ToString());
                    MySqlCommand command = new MySqlCommand(commandText, Connection);
                    MySqlDataReader dataReader;
                    dataReader = command.ExecuteReader();
                    RefreshGearTable();
